...
export default function CadastroUnidadesMedida() {
  const [itens, setItens] = useState<UnidadeMedida[]>([])
  ...
  const carregarItens = async () => {
    setCarregando(true)
    try {
      const dados = await unidadesMedidaService.listar({})
      setItens(dados)
    } catch (error) {
      setToast({ tipo: 'error', mensagem: 'Erro ao carregar Unidades de Medida' })
    } finally {
      setCarregando(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="mb-6">
        ...
        <p className="text-slate-600 mt-2">Unidades comerciais e tributárias</p>
      </div>
          ...
          <table className="min-w-full">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">Sigla</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">Descrição</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">Decimal</th>
              ...
              {itens.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    {item.permite_decimal ? `Sim (${item.casas_decimais} casas)` : 'Não'}
                  </td>

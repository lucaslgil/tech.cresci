...
export const Layout: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      {/* Sidebar */}
      <div className="w-48 sm:w-56 md:w-60 bg-slate-900 flex-shrink-0">
        <div className="flex flex-col h-full">
          {/* Header with User */}
          <div className="px-2 sm:px-3 md:px-4 py-3 sm:py-4 border-b border-slate-800">
            {/* User Profile - Clickable */}
            <Link 
              to="/configuracao"
              className="flex items-center hover:bg-slate-800 p-1.5 sm:p-2 -m-1.5 sm:-m-2 rounded-lg transition-colors group"
            >
              <div className="relative">
                <div className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 bg-slate-700 rounded-full flex items-center justify-center overflow-hidden border-2 border-slate-600 group-hover:border-slate-500 transition-colors flex-shrink-0">
                  {user?.email ? (
              ...
              <div className="ml-2 sm:ml-3 flex-1 min-w-0">
                <p className="text-white text-xs sm:text-sm font-medium truncate group-hover:text-slate-200">
                  {user?.email?.split('@')[0]}
                </p>
                <p className="text-slate-400 text-[10px] sm:text-xs truncate hidden sm:block">
                  {user?.email}
                </p>
              </div>
              <svg className="w-3 h-3 sm:w-4 sm:h-4 text-slate-500 group-hover:text-slate-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* Navigation */}
          <nav className="flex-1 px-1.5 sm:px-2 py-3 sm:py-4 space-y-1 overflow-y-auto">
            {/* Dashboard */}
            <Link
              to="/dashboard"
              className="w-full flex items-center px-2 sm:px-3 py-2 text-xs sm:text-sm font-medium rounded transition-colors duration-200 text-slate-400 hover:text-slate-300 hover:bg-slate-800"
            >
            ...
            <div>
              <button
                ...
                className="w-full flex items-center justify-between px-2 sm:px-3 py-2 text-xs sm:text-sm font-medium rounded transition-colors duration-200 text-slate-400 hover:text-slate-300 hover:bg-slate-800"
              >
                ...
                <svg 
                  className={`w-3 h-3 sm:w-4 sm:h-4 transition-transform duration-200 flex-shrink-0 ${cadastroOpen ? 'rotate-90' : ''}`}
                  fill="none" 
                  stroke="currentColor" 
                  viewBox="0 0 24 24"
                >
